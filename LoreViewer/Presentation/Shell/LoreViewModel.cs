...
namespace LoreViewer.Presentation.ViewModels
{
  public class LoreViewModel : ViewModelBase
  {
    ...
    private async Task RunParse()
    {
      ...
      ParseResult pr = await _parser.ParseFolderAsync(LoreLibraryFolderPath, _progress, ct);

      if (pr.IsFatal)
      {
        Errors.Add(new ParseErrorViewModel(new ParseError("", -1, -1, new Exception("FATAL ERROR"))));
        IsParsing = false;
        return;
      }
    ...
    #region Refreshing
    private void RefreshAllFromLoreRepo_Old()
    {
