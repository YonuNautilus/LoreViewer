...
namespace LoreViewer.Domain.Settings
{
  /// <summary>
  ...
  public class LoreSettings : IDeepCopyable<LoreSettings>
  {
    ...
    public LoreSettings Clone()
    {
      ...
      newSettings.types = types.Select(t => t.Clone()).ToList();
      newSettings.collections = collections.Select(c => c.Clone()).ToList();
      newSettings.picklists = picklists.Select(p => p.Clone()).ToList();

      newSettings.settings = settings.Clone();

      newSettings.OriginalYAML = OriginalYAML;
