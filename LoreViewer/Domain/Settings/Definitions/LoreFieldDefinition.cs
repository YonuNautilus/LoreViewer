  ...
  public class LoreFieldDefinition : LoreDefinitionBase, IFieldDefinitionContainer, IRequirable, IDeepCopyable<LoreFieldDefinition>
  {
    ...
    public override bool IsModifiedFromBase
    {
      get
      {
        ...
        if (required != (Base as LoreFieldDefinition).required) return true;

        if (structure != (Base as LoreFieldDefinition).structure) return true;

        if (HasFields)
          if (fields.Any(f => f.IsModifiedFromBase)) return true;

        if (contentType == EFieldContentType.PickList)
          if (picklistBranchRestriction != (Base as LoreFieldDefinition).picklistBranchRestriction) return true;
    ...
    public void MergeFrom(LoreFieldDefinition parentField)
    {
      ...
      if (Picklist == null && parentField.Picklist != null)
      ...
      if (RefListType == null && parentField.RefListType != null)
        RefListType = parentField.RefListType;
    ...
    {
      LoreFieldDefinition fieldDef = MemberwiseClone() as LoreFieldDefinition;
      fieldDef.fields = fields?.Select(f => f.Clone()).ToList();

      fieldDef.picklistName = picklistName;
      fieldDef.picklistBranchRestriction = picklistBranchRestriction;
