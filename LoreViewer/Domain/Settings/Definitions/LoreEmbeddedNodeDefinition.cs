  ...
  public class LoreEmbeddedNodeDefinition : LoreDefinitionBase, IRequirable, IDeepCopyable<LoreEmbeddedNodeDefinition>
  {
    ...
    public override bool IsModifiedFromBase
    {
      get
      {
        ...
        if (required != (Base as LoreEmbeddedNodeDefinition).required) return true;

        if (nodeType != (Base as LoreEmbeddedNodeDefinition).nodeType) return true;
    ...
    public void MergeFrom(LoreEmbeddedNodeDefinition baseEmbedded)
    {
      ...
      else if (!(nodeType as LoreTypeDefinition).IsATypeOf(baseEmbedded.nodeType as LoreTypeDefinition))
        nodeType = baseEmbedded.nodeType;

      required |= baseEmbedded.required;
    }

    public LoreEmbeddedNodeDefinition Clone()
    {
      LoreEmbeddedNodeDefinition emNodeDef = MemberwiseClone() as LoreEmbeddedNodeDefinition;
      emNodeDef.Base = this;
